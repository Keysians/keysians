...
fn bridge_eos_genesis_config_should_work() {
	new_test_ext().execute_with(|| {
		assert_eq!(BridgeContractAccount::get(), (b"keysiancross".to_vec(), 2));
...
fn chech_receiver_is_ss58_format() {
	...
	// this is a keysians account address
	const KEYSIANS_PREFIX: u8 = 6;
	let keysians_address = "gg2XUSNDsdmYR28YRVMZ7qeWqPpaKtG5PefahS4yKwshda2";
	let decoded_ss58 = bs58::decode(keysians_address).into_vec();
	assert!(decoded_ss58.is_ok());
	let decoded_ss58 = decoded_ss58.unwrap();
	assert_eq!(decoded_ss58.len(), 35);
	assert_eq!(decoded_ss58[0], KEYSIANS_PREFIX);

	// this is a centrifuge account address
	let keysians_address = "4bVs9EVoVRcx2BGcf66nMsv3Bw5WAwVLSzorZcUnLjLZebf7";
	let decoded_ss58 = bs58::decode(keysians_address).into_vec();
	assert!(decoded_ss58.is_ok());
	let decoded_ss58 = decoded_ss58.unwrap();
	assert_eq!(decoded_ss58.len(), 35);
	assert_ne!(decoded_ss58[0], KEYSIANS_PREFIX);
}
